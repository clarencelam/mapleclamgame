...
class Coin {
    ...
    // MODIFIED: Use mute-aware function
    playPickupSfx()
    }
...
class Food {
    constructor({ position, velocity, imageSrc, direction, scale = 1, framesMax = 1 }) {
        ...
        this.image.src = imageSrc
        this.scale = scale
        this.eatenBy = null; // new property
...
class Customer {
    constructor({ position, velocity, imageSrc, type = 'green',scale = 1, framesMax = 1, sprites }) {
        this.position = position
        this.velocity = velocity
        this.image = new Image()
        this.image.src = imageSrc
        ...
        // New: Customer type and eating counter
        this.type = type // 'green' or 'blue'
        this.eatCount = 0
        this.eatingEmoteTimeInFrames = 60
        ...
        // New: Timers for hit sprite and despawn
        this.hitTimer = 0
        this.despawnTimer = 0
    }
    ...
    eat() {
        ...
        // eat sound effect
        playEatSfx()
        // Generate coins based on customer type
        var pos_x = this.position.x + (this.width / 2)
        var pos_y = this.position.y

        if (this.type === 'green') {
            // Original Green Snail behavior: 1 coin per food
            genCoin(pos_x, pos_y - 40)
            console.log("green snail down - drop 1 coin")
            this.markedForDeath = true
        } else if (this.type === 'blue') {
            // Blue Snail: eats 3 foods, then drops 2 coins and despawns
            this.eating = false
            if (this.eatCount === 3) {
                // Drop 2 coins
                genCoin(pos_x, pos_y - 40)
                genCoin(pos_x + 20, pos_y - 40)
                console.log("dblue snail down - drop 2 coins")
                // Make Blue Snail jump off the map and despawn
                this.markedForDeath = true
                this.velocity.y = -8 // Big upward jump
                this.despawnTimer = 120 // Despawn after 120 frames
            }
            // Show hit sprite for 60 frames
            if (this.facing === -1) {
    ...
    switchSprite(sprite) {
        // Don't switch sprites while showing hit animation
        if (this.isHit && (sprite === 'walk' || sprite === 'walkRight' || sprite === 'idle' || sprite === 'idleRight')) {
            return
        }
    ...
    update() {
        ...
        // Handle hit sprite timer
        if (this.hitTimer > 0) {
            this.hitTimer--
             if (this.hitTimer === 0) {
                // Switch back to idle after hit animation
                this.eating = false
                if (this.facing === 1) {
                    this.switchSprite('idleRight')
                } else {
            ...
            } else if (this.hitTimer === 59){
                this.velocity.x = 0 // stop the cust x movement
            }
        }

        // Handle despawn timer for Blue Snails
        if (this.despawnTimer > 0) {
            this.despawnTimer--
            if (this.despawnTimer === 0) {
                // Force despawn by moving off-screen
